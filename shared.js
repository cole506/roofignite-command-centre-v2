...
async function loadCreativeSection(clientName, subfolder, key) {
  ...
  grid.innerHTML = files.map(f => `
    <div class="relative group rounded-xl overflow-hidden border border-dark-600/30 hover:border-purple-500/30 transition-all" style="aspect-ratio:1;">
      ...
      <button onclick="deleteCreativeFile('${f.id}', '${esc(clientName)}', '${subfolder}', '${key}')" class="absolute top-1 right-1 w-5 h-5 rounded-full bg-red-500/80 hover:bg-red-500 text-white flex items-center justify-center text-xs z-10 transition-all" title="Delete">×</button>
      <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center cursor-pointer" onclick="showImagePreview('${f.thumbnailUrl.replace('=w200','=w800')}', '${esc(f.name)}')">
...
function showImagePreview(url, name) {
  const overlay = document.createElement('div');
  overlay.className = 'fixed inset-0 z-[600] flex items-center justify-center';
